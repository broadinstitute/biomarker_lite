...
Expression <-load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='OmicsExpressionProteinCodingGenesTPMLogp1')
colnames(Expression) %<>% word()
Expression <- Expression[, apply(Expression, 2, function(x) var(x, na.rm = T)) > .1]


CRISPRGeneEffect <- load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='CRISPRGeneEffect')
colnames(CRISPRGeneEffect) %<>% word()
CRISPRGeneEffect <- CRISPRGeneEffect[, apply(CRISPRGeneEffect, 2,  function(x) var(x, na.rm = T)) > 0.01]



MatrixDamaging <- load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='OmicsSomaticMutationsMatrixDamaging')
MatrixHotspot <- load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='OmicsSomaticMutationsMatrixHotspot')
...
compound_list <- data.table::fread("~/Desktop/PREP/RELEASE FILES_23Q2/FINAL/PUBLIC/Repurposing_Public_23Q2_Extended_Primary_Compound_List.csv") 
compound_matrix <- data.table::fread("~/Desktop/PREP/RELEASE FILES_23Q2/FINAL/PUBLIC/Repurposing_Public_23Q2_Extended_Primary_Data_Matrix.csv") %>% 
...
Model <- load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='Model') %>% 
  dplyr::mutate(dummy = 1) %>% 
  reshape2::acast(ModelID ~ DepmapModelType, value.var = "dummy", fill = 0) 
Model <- Model[, apply(Model, 2, function(x) sum(x, na.rm = T)) > 9] 


OmicsCNGene <- load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='OmicsCNGene')
...
map <- load.from.taiga(data.name='public-23q2-19de', data.version=101, data.file='Model') %>%
...
MATRIX %>% 
  write.csv("data/BIOMARKER_FEATURES_PUBLIC.csv")
